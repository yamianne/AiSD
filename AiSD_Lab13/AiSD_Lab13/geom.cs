...
namespace ASD
{
    public static partial class Geometry
    {
        ...
        public struct Point
        {
            ...
            public static bool EpsilonEquals(Point p1, Point p2) => Distance(p1,p2)<Epsilon ;
        ...
        public static bool Intersection(Segment s1, Segment s2)
        {
            ...
            return Math.Min(Math.Max(s1.ps.x, s1.pe.x), Math.Max(s2.ps.x, s2.pe.x)) >= Math.Max(Math.Min(s1.ps.x, s1.pe.x), Math.Min(s2.ps.x, s2.pe.x)) &&
                   Math.Min(Math.Max(s1.ps.y, s1.pe.y), Math.Max(s2.ps.y, s2.pe.y)) >= Math.Max(Math.Min(s1.ps.y, s1.pe.y), Math.Min(s2.ps.y, s2.pe.y));
